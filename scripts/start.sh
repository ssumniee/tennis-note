...
export COOLSMS_API_KEY=$(aws ssm get-parameters --region ap-northeast-2 --names COOLSMS_API_KEY --query Prameters[0].Value | sed 's/"//g')
export COOLSMS_API_SECRET=$(aws ssm get-parameters --region ap-northeast-2 --names COOLSMS_API_SECRET --query Prameters[0].Value | sed 's/"//g')
export COOLSMS_CALLING_NUMBER=$(aws ssm get-parameters --region ap-northeast-2 --names COOLSMS_CALLING_NUMBER --query Prameters[0].Value | sed 's/"//g')
